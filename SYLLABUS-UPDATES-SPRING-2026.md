# STCM140 syllabus updates for Spring 2026

Based on the MSU Academic Calendar, here are the complete updates for the Spring 2026 syllabus.

---

## Course information

**Course:** STCM140 Multimedia Production for Strategic Communications
**Schedule:** Tu/Th, 10:00-11:25 AM, MRHD-143
**Semester:** January 16 - May 4, 2026
**First Tu/Th class:** Tuesday, January 20, 2026
**Last Tu/Th class:** Thursday, April 30, 2026
**Total class meetings:** 28 sessions (14 weeks × 2)

---

## Key calendar dates

| Date | Event | Impact on Tu/Th class |
|------|-------|----------------------|
| Fri, Jan 16 | First day of semester (Monday schedule) | Tu/Th classes start Jan 20 |
| Mon, Jan 19 | MLK Day | No impact |
| Mar 7-15 | Spring Break | No class Week 8 |
| Fri, Apr 3 | Good Friday | No impact |
| Mon, May 4 | Last day of semester (Friday schedule) | Final project due |

---

## Complete course schedule with dates

**Note:** Spring Break falls during Week 8 (March 7-15). The research dossier has been shifted to Week 10 to give students a full week after the topic is covered.

### Week 1: January 20-22 | Introduction & the state of media
We establish the ground rules and discuss the reality of the media business.
- **Topic:** Syllabus review, course expectations, and the "enshittification" of platforms.
- **Reading:** "Your Media Business Will Not Be Saved" by Joshua Topolsky.

### Week 2: January 27-29 | How to use your computer
Digital fundamentals for strategic communicators.
- **Topic:** Keyboard shortcuts, file organization, and professional workflows.
- **Activity:** Keyboard shortcut speed challenge and folder structure setup.
- **Due Thu, Jan 29:** Academic engagement verification
- **Due Thu, Jan 29:** Read the Cluetrain Manifesto (25 pts)

### Week 3: February 3-5 | Media markets & information needs
Understanding where your audience lives and what they actually need versus what they get.
- **Topic:** Analyzing media ecosystems and identifying coverage gaps.
- **Activity:** Market analysis workshop.

### Week 4: February 10-12 | Authenticity & the Cluetrain Manifesto
Why human voices matter more than algorithms.
- **Topic:** The 95 theses of the Cluetrain Manifesto and the allure of the "New Thing."
- **Discussion:** AI ethics and the value of genuine perspective.
- **Due Thu, Feb 12:** Media markets & information needs (20 pts)

### Week 5: February 17-19 | The attention economy
How platforms harvest attention and what it means for creators.
- **Topic:** Algorithms, privacy, and screen time metrics.
- **Guest lecture:** The mechanics of the attention economy (subject to availability).

### Week 6: February 24-26 | Principles of design
The fundamentals of visual communication: contrast, hierarchy, alignment, and balance.
- **Topic:** Why bad design hurts credibility.
- **In-class activity:** Alien planet travel poster—applying design principles to absurd constraints.
- **Due Thu, Feb 26:** Enshittification & file organization (25 pts)

### Week 7: March 3-5 | Presentations & final project kickoff
Putting design theory into practice.
- **Activity:** "Slide deck that doesn't suck" student presentations.
- **Topic:** Introduction to the final project: Multimedia asset package.
- **Due Thu, Mar 5:** Design a slide deck that doesn't suck (50 pts)
- **Due Thu, Mar 5:** Alien planet travel poster (50 pts)

### Week 8: March 7-15 | SPRING BREAK
**NO CLASS**

### Week 9: March 17-19 | Research & synthesis
Gathering intelligence before creating content.
- **Topic:** Building a research dossier and using tools like NotebookLM for synthesis.
- **Focus:** Competitor analysis and finding inspiration.

### Week 10: March 24-26 | Audience strategy & personas
Moving beyond demographics to understand motivation.
- **Topic:** Creating detailed customer/user personas.
- **Activity:** Mapping user pain points to strategic solutions.
- **Due Thu, Mar 26:** Research dossier (50 pts)

### Week 11: March 31 - April 2 | Copywriting & voice
Writing with clarity and authority.
- **Topic:** The "solutions lens"—critiquing industry problems without being toxic.
- **Reading:** Selections from On Writing Well by William Zinsser (Simplicity and Clutter).
- **Due Thu, Apr 2:** User/customer persona (75 pts)

### Week 12: April 7-9 | Visuals for the web
Designing for the feed and the article.
- **Topic:** Blog posts, featured images, and aspect ratios.
- **Workshop:** Canva best practices for high-resolution assets.
- **Due Thu, Apr 9:** Critical copywriting (100 pts)

### Week 13: April 14-16 | Social graphics & accessibility
Creating standalone assets that convert.
- **Topic:** Social cards, call-to-actions (CTAs), and the necessity of alt text.
- **Focus:** Designing for accessibility and mobile screens.

### Week 14: April 21-23 | Branding & consistency
Ensuring your organization looks the same everywhere.
- **Topic:** Style guides, branding kits, and defining visual rules.
- **Activity:** Building the "instruction manual" for your brand.
- **Due Thu, Apr 23:** Featured images + social graphics (75 pts)

### Week 15: April 28-30 | Final project workshop
Crunch time.
- **Topic:** Peer review, troubleshooting, and polishing deliverables.
- **Focus:** Refining the asset package and ensuring all links work.
- **Due Thu, Apr 30:** Campaign strategy (75 pts)

### Week 16: Finals period | Final presentations
- **Activity:** Final project overview and course wrap-up.
- **Due Mon, May 4:** Final project: Multimedia asset package (~40% of grade)

**Note:** Week 16 falls during the finals period. Check the registrar for your assigned final exam time slot.

---

## Complete schedule of assignments

| Due date | Assignment | Points |
|----------|------------|--------|
| Thu, Jan 29 | Academic engagement verification | — |
| Thu, Jan 29 | Read the Cluetrain Manifesto | 25 |
| Thu, Feb 12 | Media markets & information needs | 20 |
| Thu, Feb 26 | Enshittification & file organization | 25 |
| Thu, Mar 5 | Design a slide deck that doesn't suck | 50 |
| Thu, Mar 5 | Alien planet travel poster | 50 |
| Thu, Mar 26 | Research dossier | 50 |
| Thu, Apr 2 | User/customer persona | 75 |
| Thu, Apr 9 | Critical copywriting | 100 |
| Thu, Apr 23 | Featured images + social graphics | 75 |
| Thu, Apr 30 | Campaign strategy | 75 |
| Mon, May 4 | Final project: Multimedia asset package | ~40% of grade |

**Total points before final project:** 545 pts

---

## New assignment descriptions

### Alien planet travel poster (50 pts)
**Due:** Thursday, March 5

Using the design principles covered in Week 6, create a travel poster for a fictional alien planet. This assignment forces you to apply contrast, hierarchy, alignment, and balance to content you can't rely on familiarity to make work. The poster should:
- Be print-ready at 11x17 inches, 300 DPI
- Include a compelling headline, tagline, and at least one visual element
- Demonstrate clear visual hierarchy and intentional use of color
- Feel like something you'd actually see hanging in a travel agency (on another planet)

This is completed as an in-class activity during Week 6 and due by the start of class the following Thursday.

### Featured images + social graphics (75 pts)
**Due:** Thursday, April 23

Create a cohesive set of visual assets for your final project organization. This bundle includes:
- **3 featured images** for blog posts or articles (1200x630 pixels, optimized for web)
- **3 social media graphics** (square format, 1080x1080 pixels)

All six graphics must:
- Follow consistent branding (colors, fonts, visual style)
- Include proper alt text descriptions
- Be export-ready for web use (PNG or JPEG, optimized file size)

This assignment directly feeds into your final multimedia asset package.

### Campaign strategy (75 pts)
**Due:** Thursday, April 30

Write a 2-3 page strategic document that outlines how your organization would deploy the assets you've created throughout the semester. Include:
- **Campaign goal:** What are you trying to achieve?
- **Target audience:** Who are you reaching (reference your persona)?
- **Platform strategy:** Where will these assets live and why?
- **Content calendar:** A two-week sample showing when and where each asset type would be posted
- **Success metrics:** How will you measure whether this worked?

This is the "instruction manual" that explains how all your semester work fits together.

---

## Class meeting dates (all Tuesdays and Thursdays)

### January 2026
- Tue 1/20, Thu 1/22 (Week 1)
- Tue 1/27, Thu 1/29 (Week 2)

### February 2026
- Tue 2/3, Thu 2/5 (Week 3)
- Tue 2/10, Thu 2/12 (Week 4)
- Tue 2/17, Thu 2/19 (Week 5)
- Tue 2/24, Thu 2/26 (Week 6)

### March 2026
- Tue 3/3, Thu 3/5 (Week 7)
- ~~Tue 3/10, Thu 3/12~~ Spring Break (Week 8)
- Tue 3/17, Thu 3/19 (Week 9)
- Tue 3/24, Thu 3/26 (Week 10)
- Tue 3/31 (Week 11)

### April 2026
- Thu 4/2 (Week 11)
- Tue 4/7, Thu 4/9 (Week 12)
- Tue 4/14, Thu 4/16 (Week 13)
- Tue 4/21, Thu 4/23 (Week 14)
- Tue 4/28, Thu 4/30 (Week 15)

### May 2026
- Finals period (Week 16) — Final project due Mon 5/4

**Total class meetings:** 28 sessions

---

## Summary of changes from original syllabus

### 1. Spring Break moved to Week 8
The original syllabus had Spring Break during Week 9, but March 7-15 actually falls during Week 8 of the semester. This has been corrected.

### 2. Research dossier shifted to Week 10
Previously due during Week 9 (the same week the topic was covered), the research dossier is now due during Week 10. This gives students a full week after learning about research and synthesis to complete the assignment.

### 3. User persona shifted to Week 11
With the research dossier moving to Week 10, the user/customer persona assignment now falls due during Week 11 (was Week 10). This maintains the logical progression: research first, then persona development.

### 4. Three new graded assignments added

| Assignment | Points | Rationale |
|------------|--------|-----------|
| Alien planet travel poster | 50 | Converts the in-class design activity into a graded deliverable |
| Featured images + social graphics | 75 | Fills the gap in Weeks 13-14 and creates portfolio-ready assets |
| Campaign strategy | 75 | Ties the semester's work together with strategic thinking |

### 5. Point total increased
The total points before the final project increased from 345 to 545 pts with the addition of three new assignments worth 200 points combined.

### 6. Assignment spacing improved
No assignments are due the same week their topic is introduced. Students have at least one class session between learning content and submitting related work.

---

## Verification checklist

- [x] All 28 class sessions accounted for (14 weeks × 2 sessions, minus 2 for Spring Break)
- [x] No assignments due during Spring Break (Week 8)
- [x] Assignment due dates follow topic coverage (not same day)
- [x] Point totals are consistent (545 pts before final)
- [x] New assignment descriptions included
- [x] Complete week-by-week schedule with dates
- [x] All assignment due dates specified
